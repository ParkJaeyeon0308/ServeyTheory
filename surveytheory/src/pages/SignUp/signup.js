...
export default class SignUp extends React.Component {
    ...
    onclick = () => {
        const textbox = {
          ...
          // 중복체크 안했으면 하라고 출력 ( 중복체크를 할 시에는 data에 값이 들어가있음 )
           ...
           // 비밀번호끼리 같고 data가 중복이 아니면 넘김 + alert창 ( 위의 유효성 검사를 다 통과했으면 )
            ...
            fetch("http://localhost:3001/text", { // server.js의 text 메소드 ( 정보 insert )
                method: "post", //통신방법
                headers: {
      ...
      onclick1 = () => {
        ...
        fetch("http://localhost:3001/data", { // server.js의 data 메소드 ( user_id가 중복되는 행의 개수 )
          method: "post", //통신방법
          headers: {
            "content-type": "application/json",
          },
          ...
          .then((json) => {
            console.log(json);
            if(json.count == '1'){ // count는 db에서 select 했을 때 출력되는 컬럼명 ( 중복되는 행의 개수가 1이면 )
                this.setState({
